...
<!-- 预约时间区域 -->
        ...
        <div class="vp-section">
          ...
          <div class="vp-diy-level1" style="display:flex;gap:4px;margin-bottom:12px">
            <button class="vp-diy-l1 active" style="padding:8px 16px;border:1px solid var(--border);border-radius:6px;cursor:pointer;font-size:0.9em;background:var(--primary);color:white" onclick="document.getElementById('vp-diy-plan-section').style.display='block';document.getElementById('vp-diy-upload-section').style.display='none'">🗺️ 行程制定</button>
            <button class="vp-diy-l1" style="padding:8px 16px;border:1px solid var(--border);border-radius:6px;cursor:pointer;font-size:0.9em" onclick="document.getElementById('vp-diy-plan-section').style.display='none';document.getElementById('vp-diy-upload-section').style.display='block'">📄 材料上传</button>
          </div>
          <!-- Plan section -->
          <div id="vp-diy-plan-section">
            <div class="vp-diy-tabs" style="margin-bottom:8px">
              <button class="vp-diy-tab active">📋 行程单</button>
              <button class="vp-diy-tab">🏨 酒店</button>
              <button class="vp-diy-tab">✈️ 机票</button>
              <button class="vp-diy-tab">🏥 保险</button>
              <button class="vp-diy-tab">🌐 翻译</button>
            </div>
  ...
  function switchDIYLevel1(level, countryId, visaType) {
    ...
    // Find the button whose text matches the selected level
    btns.forEach(function(b) {
      if ((level === 'plan' && (b.textContent.includes('行程') || b.textContent.includes('制定'))) ||
          (level === 'upload' && b.textContent.includes('材料'))) {
  ...
  function switchPlanTab(tab, countryId, visaType) {
    ...
    if (parent) {
      parent.querySelectorAll('.vp-diy-tab').forEach(function(t) { t.classList.remove('active'); });
    }
...
//     if (!c) return;
//     var today = new Date();
//     var weekLater = new Date(Date.now() + 7 * 86400000);
//     c.innerHTML = '<div class="vp-diy-itinerary"><h4>📋 生成旅行行程单</h4>' +
//       '<div class="vp-form-row"><label>出发日期：<input type="date" class="vp-input" id="vp-it-start" value="' + today.toISOString().slice(0,10) + '"></label>' +
//       '<label>返回日期：<input type="date" class="vp-input" id="vp-it-end" value="' + weekLater.toISOString().slice(0,10) + '"></label></div>' +
